...
void sortRecordsByID(EmployeeRecord records[])
{
    ...
    for (i = 0; i < (EMPLOYEES - 1); i++)
    {
        ...
        {
            if (records[j].id_num < min.id_num)        //if we’ve located a lower value, capture it
            {
                min = records[j];
                minidx = j;
            }
        }
        if (min.id_num < records[i].id_num)    //check of we have a new minimum and if we do, swap
        {
            temp = records[i];
            records[i] = min;
            records[minidx] = temp;
